...
const getTransactions = () => {
  return new Promise((resolve, reject) => {
    const query =
      "select users.id_user,id_transaction, products.image_product, products.name_product, products.price, amount, address, phone_number, status,userdata.display_name, transactions.size from transactions INNER JOIN products ON transactions.id_product=products.id_product INNER JOIN users ON transactions.id_user=users.id_user INNER JOIN userdata ON users.id_user=userdata.id_user";
